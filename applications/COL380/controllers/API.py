...
def SendStatus():
    def POST(*args, **vars):
        ...
        if (int(request.vars['status']) == 1):
            row.update_record(CompiledAt=request.now, TestCase1Log=request.vars['O1'], TestCase1Error= request.vars['E1'], Status="Test case ran successfully!", Score=int(request.vars['score']))
        else:
